...
class basic_building_blocks:
    ...
    def RandInt(self, int k, int n=1):
    ...
    # @jit(fastmath=True)
    def Mul(self, np.ndarray x, np.ndarray y):
    ...
    def MulPub(self, np.ndarray x, np.ndarray y):
    ...
    def XOR(self, np.ndarray[DTYPE64, ndim=2] a, np.ndarray[DTYPE64, ndim=2] b):
        return a + b - 2 * self.Mul(a, b)

    def OR(self, np.ndarray[DTYPE64, ndim=2] a, np.ndarray[DTYPE64, ndim=2] b):
        return a + b - self.Mul(a, b)

    def Inv(self, np.ndarray[DTYPE64, ndim=2] a):
        """ Return 1/a
    ...
    # @jit(fastmath=True)
    def PreMul(self, np.ndarray[DTYPE64, ndim=3] x):
    ...
    # @jit(fastmath=True)
    def PreOr(self, np.ndarray[DTYPE64, ndim=3] a):
    ...
    # @jit(fastmath=True)
    def KOrCS(self, np.ndarray[DTYPE64, ndim=3] a):
    ...
    # @jit(fastmath=True)
    def KOr(self, np.ndarray[DTYPE64, ndim=3] a):
    ...
    # @jit(fastmath=True)
    def Mod2(self, np.ndarray[DTYPE64, ndim=2] x, int k):
    ...
    # @jit(fastmath=True)
    def BitLT(self, np.ndarray[DTYPE64, ndim=3] a, np.ndarray[DTYPE64, ndim=3] b):
    ...
    # @jit(fastmath=True)
    def Mod2m(self, np.ndarray[DTYPE64, ndim=2] x, int k, int m):
        """ Compute x mod 2^m
    ...
    # @jit(fastmath=True)
    def Mod2m_shared_m(self, np.ndarray[DTYPE64, ndim=2] x, int k, np.ndarray[DTYPE64, ndim=2] m):
    ...
    # @jit(fastmath=True)
    def Trunc(self, np.ndarray[DTYPE64, ndim=2] x, int k, int m):
        """ Coupute x / 2^m
    ...
    # @jit(fastmath=True)
    def EQZ(self, np.ndarray[DTYPE64, ndim=2] x, int k):
    ...
    def LTZ(self, np.ndarray[DTYPE64, ndim=2] x, int k):
    ...
    def EQ(self, np.ndarray x, np.ndarray y, int k):
        """ Determine if x == y or not

        """
        return self.EQZ(np.int64(x - y), k)

    def LT(self, np.ndarray[DTYPE64, ndim=2] x, np.ndarray[DTYPE64, ndim=2] y, int k):
        """ Determine if x < y or not
    ...
    # @jit(fastmath=True)
    def BitAdd(self, np.ndarray[DTYPE64, ndim=2] a, np.ndarray[DTYPE64, ndim=3] b):
    ...
    # @jit(fastmath=True)
    def BitDec(self, np.ndarray[DTYPE64, ndim=2] x, int k, int m):
    ...
    # @jit(fastmath=True)
    def Pow2(self, np.ndarray[DTYPE64, ndim=2] x, int k):
    ...
    # @jit(fastmath=True)
    def B2U(self, np.ndarray[DTYPE64, ndim=2] x, int k):
    ...
    # @jit(fastmath=True)
    def Trunc_shared_m(self, np.ndarray[DTYPE64, ndim=2] x, int k, np.ndarray[DTYPE64, ndim=2] m):
        """ Perform trunction of [x] by an unknown number of bits [m], same as x // m, where k is the bitlength of x
            x.shape = (num_integers, num_parties)
        """
        cdef np.ndarray[DTYPE64, ndim=3] m_unary_bits = self.B2U(m, k)
        cdef np.ndarray[DTYPE64, ndim=2] m_pow2_inv = self.Inv(self.Pow2(m, k))
        ...
        #cdef np.ndarray[DTYPE64, ndim=2] result_int = np.frompyfunc(int,1,1)(result)
        #result_int[:,0] += np.frompyfunc(int,1,1)(np.sum(result - result_int, axis=1))
        cdef np.ndarray[DTYPE64, ndim=2] result_int = np.int64(int,1,1)(result)
        result_int[:,0] += np.int64(np.sum(result - result_int, axis=1))
    ...
    # @jit(fastmath=True)
    def TruncPr(self, np.ndarray[DTYPE64, ndim=2] x, int k, int m):
    ...
    # @jit(fastmath=True)
    def SDiv(self, np.ndarray[DTYPE64, ndim=2] a, np.ndarray[DTYPE64, ndim=2] b, int k):
